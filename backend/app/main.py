...
def create_manual_application(req: ManualAppCreate, db: Session = Depends(get_db)):
    ...
    if req.job:
        ...
        new_job = Job(
            ...
            date_posted=(req.job.date_posted.isoformat() if req.job.date_posted else None),
...
def get_jobs_summary(db: Session = Depends(get_db)):
    ...
    try:
        ...
        base_url_counts = dict(sorted(base_url_counts.items(), key=lambda x: x[1], reverse=True))
